...
export class BuildTheWordScene extends Phaser.Scene {
  private crow?: Crow;
  ...
  private redrawEnvironment(w: number, h: number) {
    ...
    if (this.textures.exists('hills-btw')) { this.textures.remove('hills-btw'); }

    const ct = this.textures.createCanvas('hills-btw', w, h);
    ...
    ct.refresh();
    this.hillsImage = this.add.image(0, 0, 'hills-btw').setOrigin(0, 0);
